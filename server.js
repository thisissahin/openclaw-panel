...
const GITHUB_REPO = 'thisissahin/openclaw-panel';
...
async function checkForUpdate() {
  try {
    ...
    const headers = { 'User-Agent': 'openclaw-panel', ...(token ? { Authorization: `Bearer ${token}` } : {}) };
    ...
    if (latestVersion && latestVersion !== CURRENT_VERSION) {
      ...
      console.log(`[panel] Update available: v${CURRENT_VERSION} → v${latestVersion}`);
    }
  } catch { /* no network or private repo without token — silently skip */ }
}
...
app.get('/api/ping', (_, res) => res.json({ ok: true, version: CURRENT_VERSION }));
...
app.get('/api/version', (_, res) => {
  res.json({ current: CURRENT_VERSION, latest: latestVersion, updateAvailable });
});

app.post('/api/update', (req, res) => {
  res.json({ ok: true, message: 'Update started — panel will restart shortly.' });
  setTimeout(() => {
    try {
      execSync('git fetch origin main', { cwd: __dirname, stdio: 'inherit' });
      execSync('git reset --hard origin/main', { cwd: __dirname, stdio: 'inherit' });
      execSync('npm install', { cwd: __dirname, stdio: 'inherit' });
      execSync('npm run build', { cwd: __dirname, stdio: 'inherit' });
      console.log('[panel] Update complete — restarting…');
    } catch (e) {
      console.error('[panel] Update failed:', e.message);
    }
    process.exit(0); // process manager (systemd/pm2) will restart
  }, 300);
});
